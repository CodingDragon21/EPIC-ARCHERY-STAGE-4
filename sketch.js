...
// Write code to detect collision between player arrow and opponent
 for (var i = 0; i < playerArrows.length; i++){
var cBaseCollision = Matter.SAT.collides(
  playerArrows[i].body,
  computerBase.body,


);
var cArcherCollision = Matter.SAT.collides(
  playerArrows[i].body,
  computer.body,

);
var cArrowCollision = Matter.SAT.collides(
  playerArrows[i].body,
  computerArcher.body,

);
if(
  cBaseCollision.collided ||
  cArcherCollision.collided||
  cArrowCollision.collided
){

  console.log("YOU HIT THE COMPUTER PLAYER!")
...
function handleComputerArrowCollision() {
  ...
  for (var j = 0; j < computerArrows.length; j++){
    var pBaseCollision = Matter.SAT.collides(
    ...
    var pArcherCollision = Matter.SAT.collides(
      computerArrows[j].body,
      player.body,
      
    );
    var pArrowCollision = Matter.SAT.collides(
      computerArrows[j].body,
      playerArcher.body,
    ...
    if(
      pBaseCollision.collided ||
      pArcherCollision.collided||
      pArrowCollision.collided
    ){
      console.log("YOU GOT HIT BY THE COMPUTER!")
      
      Matter.World.remove(world,computerArrows[j].body)
      computerArrows.splice(j,1)
